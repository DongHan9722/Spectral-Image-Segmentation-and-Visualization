...
def gui_generate_img(model_gui, img_dir_gui, out_dir):

    gpu_options = tf.compat.v1.GPUOptions(allow_growth=True)
    session = tf.compat.v1.InteractiveSession(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options))

    model_name = model_gui
    dir_name = img_dir_gui
    ...
    if model_name == 'unet3plus':
        ...
        # prediction_model.load_weights('saved_models/3p_complete0.h5')
    ...
    for i in range(y_pred.shape[0]):
        if not os.path.exists(f'{out_dir}/{filenames[i]}'):
            os.makedirs(f'{out_dir}/{filenames[i]}')
        ...
        plt.draw()
        plt.pause(0.001)
        plt.imsave(f'{out_dir}/{filenames[i]}/{filenames[i]}.png', y_pred_label_plot[i], cmap = cmap)
        for j in range(num_labels):
            cmap_sub = matplotlib.colors.ListedColormap(colors_individual[j])
            foo = label_names1.index(label_names[j])
            plt.imsave(f'{out_dir}/{filenames[i]}/{foo}.png', y_pred_class_plot[i][j], cmap = cmap_sub)


# gui_generate_img('swinUnet', 'DataSets/OriginalDataSet/Set_1_images', 'SegmentationResults/swinUnet')
# gui_generate_img('unet3plus', 'DataSets/OriginalDataSet/Set_1_images', 'SegmentationResults/unet3plus')
